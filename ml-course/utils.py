...
class GradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        :return: weight difference: np.ndarray
        """
        self.w = self.w - gradient * self.eta(iteration)
        return gradient * self.eta(iteration)
...
class StochasticDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray, iteration: int) -> np.ndarray:
        ...
        :return: weight difference: np.ndarray
        """
        diff = gradient * self.eta(iteration)
        self.w = self.w - diff
        return diff
    ...
    def calc_gradient(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        X_restricted = X[idx, :]
        gradient = np.dot(2 * X_restricted.T, np.dot(X_restricted, self.w) - y[idx]) / X_restricted.shape[0]
        return gradient
...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> LinearRegression:
        ...
        while iter_count < self.max_iter:
            self.calc_loss(X, y)
            diff = self.descent.step(X, y, iter_count)
            if np.sum(np.square(diff)) < self.tolerance:
                return self
            iter_count += 1
        return self
    ...
    def calc_loss(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        :param y: objects' target
        """
        self.loss_history.append(np.square(np.linalg.norm(np.dot(X, self.descent.w) - y)) / X.shape[0])
